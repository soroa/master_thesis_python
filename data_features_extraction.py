...
def extract_features(X):
    ...
    mean_matrix = np.mean(X, axis=1)
    variance = np.var(X, axis=1)
    std_dev_matrix = np.std(X, axis=1)
    max_matrix = np.max(X, axis=1)
    min_matrix = np.min(X, axis=1)
    skewness_matix = skew(X, axis=1)
    kurtosis_matrix = kurtosis(X, axis=1)
    mean_spectral_energy = np.mean(np.abs(np.fft.fft(X, axis=1)), axis=1)
...
def extract_features_for_single_reading(reading):
    ...
    mean_spectral_energy = np.mean(np.abs(np.fft.fft(reading, axis=1)))
